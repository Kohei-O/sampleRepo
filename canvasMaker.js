...
//realPoint は変換したい点の実座標[x,y]
    var x = start[0] + realPoint[0]*(end[0] - start[0]) / canvasWidth;
	var y = end[1] - realPoint[1] * (end[1] - start[1]) / canvasHeight;
	return [x, y];
}
...
function drawLine(args){
    ...
    ctx.beginPath();
	ctx.moveTo(getZahyou(args.between[0])[0], getZahyou(args.between[0])[1]);
    var i = 1;
    var mLength = args.between.length;
	for (i = 1; i <= mLength - 1; i++) {
		ctx.lineTo(getZahyou(args.between[i])[0], getZahyou(args.between[i])[1]);
	}
	ctx.stroke();
    if (args.polygon === true){
        ctx.closePath();
        ctx.stroke();
    } else if (typeof(args.polygon) === "string") {
        ctx.closePath();
        ctx.stroke();
        ctx.fillStyle = args.polygon;
        ctx.fill();
    }
	baseCFL();
}
...
function drawEnko(args){
    ...
    var radius = args.radius * canvasWidth / (end[0] - start[0]);//半径はx方向の縮尺を基に定める
    ctx.beginPath();
	  if (args.clock === true) {

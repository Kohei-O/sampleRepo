var ctx;//ctx はグローバル変数 canvasMaker.js 内でも用いる 
...
var promise = new Promise(func0);

promise.then(func1).then(func2).then(func3).then(func4).then(func5).catch(reason => {console.dir(reason)});

function func0(resolve){//ナビゲーションの作成
    var topicArray = [
        '数と式',
        '論理と集合',
        'ニ次関数',
    ...
    var button1 = document.createElement("button");
            ...
            for (let k = 0; k < categoryNum; k++){
                ...
                newButton.innerHTML = topicArray[k];
            ...
            button6.style = "text-align:left; border-color: rgba(0, 0, 0, 0.125);"
            ...
            img5.style = "width:20px;margin-right:5px";
            var span7 = document.createElement("span");
            form.appendChild(span7);
            span7.style="font-size: 20px;"
...
function func1(){//データベースへの接続
return new Promise(
        function(resolve){
            var request = window.indexedDB.open("mathematics"); 
            request.onsuccess = function (event) {
                var db = event.target.result;
                resolve(db);//データベースを func2 へ渡す
            }
...
function func2(db){//ブックマーク，履歴の情報がついているIDについては, その情報をデータベースから獲得
    return new Promise(
        function(resolve){
            ...
            request.onsuccess = function(event) {
                ...
                resolve(personalData);//personalDataをfunc3 へと渡す      
            }
        }
         request.onerror = (event) => {throw event.target.error};
    }
...
function func3(personalData){//クエリの獲得
    return new Promise(
        function(resolve){
            var request1 = window.indexedDB.open("mathematics"); 
            request1.onsuccess = function (event) {
                var db = event.target.result
                var request2 = db.transaction(["query"], "readonly").objectStore("query").get(0);
                request2.onsuccess = function(event){
                    var query = event.target.result;
                    db.close();
                    resolve([personalData, query]);//personalData, query をfunc4へ渡す
            }
        request2.onerror = (event) => {throw event.target.error};
    }
        request1.onerror = (event) => {throw event.target.error};
    }
    )
}
function func4(value){//クエリに応じた問題の選択をする

    var [personalData, query] = value;
    
    var tagsArray = query.tags;
    
    if (tagsArray[0] === 'searchID'){
        var serchID = tagsArray[1];
    }
        return new Promise(
        function(resolve){    
            if (tagsArray[0] === 'searchID') {//IDで検索の時 tagsArrayは ['searchID', ID]
                let ID = tagsArray[1];
                var mondai = allMondai.filter(element => element.ID === ID);
                if (mondai.length === 0){
                    alert("指定した ID の問題はありません");
                    window.location.href = "http://mathmedia.information.jp/index.html";
                } else {
                        resolve([mondai, personalData]);//func5 へmondai, personalData]をわたす
                }
            } else if (tagsArray[0] === 'history'){//履歴から選ぶとき
                let i_end = personalData.length;
                let recordArray = [];
                for (let i = 0; i < i_end; i++){
                   let j = personalData[i].record.length - 1;
                   if (j >= 0){//record が一つ以上含まれている場合に表示する
                    let lastDate = personalData[i].record[j]; //配列 record の最新のタイムスタンプを獲得
                    recordArray.push(
                ...
                //console.dir(recordArray)
                recordArray.sort((a, b) => b.lastDate - a.lastDate);//lastDate が降順になるようソートする．どの2つの要素に対しても正すなわち真をかえすように
                let k_end = recordArray.length;
                var mondai = [];
                for (let k = 0; k < k_end; k++){
                    let added = allMondai.filter(element => element.ID === recordArray[k].ID);
                    mondai = mondai.concat(added);
                };
                console.dir(mondai);
                resolve([mondai, personalData]);

            } else if (tagsArray[0] === 'bookmark'){//ブックマークした問題を探すとき
                    var bookmarked = personalData.filter(element => element.bookmark);
                if (bookmarked.length === 0){
                    alert("ブックマークした問題はありません");
                    window.location.href = "http://mathmedia.information.jp/index.html";
                } else {
                    var mondai = [];
                    bookmarked.forEach(
                        function(element){
                            var ID = element.ID;
                            var added = allMondai.filter(element => element.ID === ID);
                            mondai = mondai.concat(added);
                        }
                     );
                     resolve([mondai, personalData]);
                }
            } else{//キーワードや分野名，難度のタグで検索したときの処理
                var tagsNum = tagsArray.length;
                var mondai = allMondai;
                for (let m = 0; m < tagsNum; m++){//該当するタグを含むものを次々ふるいにかける
                    var word = tagsArray[m];
                     mondai = mondai.filter(element => element.tags.includes(word));
                }
                if (mondai.length === 0){
                    alert("該当する問題はありません");
                    window.location.href = "http://mathmedia.information.jp/formula.html";
                } else {
                    mondai.sort((a, b) => a.ID - b.ID);//ID が昇順になるようソートする．どの2つの要素に対しても正すなわち真をかえすように
                    resolve([mondai, personalData]);
                }
            } 
        }
...
var page_nav2;
var nav_ul1;
var nav_ul2;//同じノードを二つの親要素に追加することはできず複製のため
function func5(value){//ページを表示していく    
    var [mondai, personalData] = value;
    var j_end = mondai.length ;
    //======ページネーションの作成=================-====================
    page_end = Math.floor(j_end/ 10);//特例でグローバル変数
    page_nav1 = document.createElement("nav");
    nav_ul1　= document.createElement("ul");
    page_nav1.appendChild(nav_ul1);
    document.body.appendChild(page_nav1);
    //===問題表示領域 mainBoxの作成============================================================
    ...
    for (var j = 0; j < j_end; j++) {//レコード１つごとにセクション:問題?を新設しm:mainViewに付与していく
        ...
        var page = Math.floor(j / 10);//ページ番号の獲得
        console.log('page=' + page);
        var card = document.createElement("section");
        mainBox.appendChild(card);
        card.className = "card mb-4 mt-4 all_page page" + page;
...
　      var his_list = document.createElement("ul");
        ...
        s1.appendChild(tab3);
        //==========フッター の作成==============
        var tagsNum = oneMondai.tags.length;//キーワード数を取得
        ...
        for (var n = 0; n < tagsNum; n++) {//各タグについて行う
            ...
            var formulaArray =[];
            if (formulaArray.length === 0){//公式集にないタグ(分野名，フリーワードなど)のとき
                ...
                key_button.tagsArray = [formulaArray[0].word]; 
    ...
    //==========-ページネーションの作成================================
    page_nav2 = document.createElement("nav");
    nav_ul2　= document.createElement("ul");
    page_nav2.appendChild(nav_ul2);
    document.body.appendChild(page_nav2);
    pageControl(0);
    MathJax.typesetPromise();
}
function makePagenation(page){
    ///元あった nav_ul ノードの削除
    page_nav1.removeChild(nav_ul1);//page_nav, nav_ul はグローバル変数
    page_nav2.removeChild(nav_ul2);
    //==========ページネーションの作成・更新======================
    //ページ数が2以上の時に行う
    if (page_end > 0){
        nav_ul1= document.createElement("ul");
        nav_ul1.className = "pagination justify-content-center mt-3";
            for (var i = 0; i <= page_end; i++) {
                ...
                anchor.innerHTML = i;
                anchor.href = "javascript:pageControl(" + i + ")";
                li.appendChild(anchor);
                nav_ul1.appendChild(li);
            }
        page_nav1.appendChild(nav_ul1);
        nav_ul2 = nav_ul1.cloneNode(true);//true なのでこの要素も複製
        page_nav2.appendChild(nav_ul2);
    }
} 
function pageControl(pageNumber){
    var obj = document.getElementsByClassName("all_page");
    var len = obj.length;//
    for (var k  = 0; k < len; k++){
        //foreach は配列用，for k in は他のプロパティにもアクセスしてしまう?自分で定義した簡単なオブジェクトならいいようだが
            obj[k].style.display = "none";
    }
     obj = document.getElementsByClassName("page" + pageNumber);
     len = obj.length;
     for (var k = 0 ;k < len; k++){
         obj[k].style.display = "block";
     }
     //======ページネーションの更新=================
     makePagenation(pageNumber);
}
...
function searchID(){
    ...
    var tagsArray = ['searchID', string];//['searchID', ID]
    var request = window.indexedDB.open("mathematics"); //データベースへの接続
    request.onsuccess = function () {
        var objectStore = request.result.transaction(["query"], "readwrite").objectStore("query");
        var data = {tags: tagsArray, ID: 0};//queryのレコードは ID:0 のみ。
        var secondRequest  = objectStore.put(data);
        secondRequest.onsuccess = function () {
            window.location.href = "http://mathmedia.information.jp/mathView.html";
        }
